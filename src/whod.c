...
#define MUDNAME "Theives World"

/* *** The following statement indicates the WHOD default mode       *** */
#define DEFAULT_MODE (SHOW_OFF)

/* *** The following statement tells if a character is WIZ_INVIS     *** */
/* #define IS_INVIS(ch) 0 */

/* *** Specify the lowest wizard level                               *** */
#define WIZ_MIN_LEVEL TRUST_GRUNT
#define IMP_MIN_LEVEL TRUST_IMP

/* *** Minimum level at which invis matters                          *** */
#define WIZ_MIN_INVIS TRUST_GRUNT
...
/*     The following statements will have to be added to the file        *
*      comm.c in the Diku Mud system:                                    *
*									 *
...
*      ..                                                                *
*                                                                        *
*       Apart from these changes, the function do_whod should also       *
*       be added as a command in the file interpreter.c                  *
*	  In the bottom of this file, there is a suggestion to what	 *
*	you might want as help in the help_file				 *
*                                                                        *
\************************************************************************/
...
void whod_loop(void)
{
    int	    nfound, players = 0, gods = 0, count, level;
    socklen_t     size;
    fd_set  in;
    u_long  hostlong;
    long    plyr_class;
    ...
    char    buf[MAX_STRING_LENGTH], tmp[80], tmpbuf[80];
    struct  char_data *ch;
    struct  hostent *hent;

    static  int newdesc;
    static  char clss[MAX_CLASS] = {'M','C','W','T','P','D','S','R','H'};
	...
	if (FD_ISSET(s,&in))
	{
		...
		sprintf(buf,"WHO request from %ld.%ld.%ld.%ld served.", 
		...
		{
		    if ( (IS_SET(SHOW_INVIS,whod_mode) &&
			 (TRUST(ch) < WIZ_MIN_INVIS)) ||
			(ch->invis_level == 0) )
		    {
		        tmp[0]='\0';
			if (IS_SET((SHOW_LEVEL | SHOW_CLASS),whod_mode))
			{
				...
				(TRUST(ch) >= WIZ_MIN_LEVEL) &&
				IS_SET(ch->specials.flags, PLR_NOWIZ))
			    {
				if (TRUST(ch)>=IMP_MIN_LEVEL)
				  strcat(tmp, "<<<IMP>>>");
				else if (IS_SET(SHOW_LEVEL,whod_mode) &&
				...
				{
				  plyr_class=GET_CLASS(ch);
				  count=0;
				  
				  for(i=0;i<MAX_CLASS;i++)
				  {
				    if(IS_SET(1<<i,plyr_class))
				    {
				      sprintf(tmpbuf,"%c",clss[i]);
				      strcat(tmp,tmpbuf);
				      count++;
				      if(count<HowManyClasses(ch)) 
					strcat(tmp,"/");
				    }
				  }
				}
			    ...
			    if  (!ch->desc)
			      sprintf(tmp,"LINKDEAD");
			    spaces=(21-strlen(tmp))/2;
         		    for(i=1;i<(21-spaces-strlen(tmp));i++)
			      strcat(buf," ");
			    strcat(buf,tmp);
			    for(i=1;i<spaces;i++)
			      strcat(buf," ");
			...
			{
			  sprintf(tmp,"%-16s",GET_REAL_NAME(ch));
			  strcat(buf,tmp);
			}
