                 ...
                 char mes1[MAX_STRING_LENGTH], char mes2[MAX_STRING_LENGTH])
{
  ...
  EACH_CHARACTER(iter, vict)
	...
	{
	  if (ch!=vict || (under_water_electric_attack)) {
	    if(IS_IMMORTAL(vict))
	      send_to_char( "Some puny mortal tries to hurt you.\n\r"
			    , vict);
	    else
	      {
		// Area damage can be done to ANYONE who is not either following
		// the caster or the caster is following him.  
		// A player stupid enough to wander in to the room anyway, will
		// receive the full blast of damage.
		if(!is_following(ch,vict))
                      ...
                      struct char_data *victim, struct obj_data *obj)
{
  ...
  for(i=0; i < num; i++)
  {
    ...
    if( damage(ch, victim, dam, SPELL_ACID_BLAST)) i=num;
    else {
                        ...
                        struct char_data *victim, struct obj_data *obj)
{
  ...
  for(i=0; i < num; i++)
  {
     if(IS_PURE_CLASS(ch))
        ...
        dam=dice(level, 8);
      /*** check for immunity and backfire spell if set ***/
  if (ImpSaveSpell(victim, saveType, energyDrainSpellMod) && !ImpSaveSpell(ch, saveType, energyDrainSpellMod) &&
      !IS_IMMORTAL(ch)) {
    if (number(1,4)==1) {
     send_to_char("Your energy drain backfires!\n\r",ch);
      if (ImpSaveSpell(ch, saveType,energyDrainSpellMod ))
        dam /=2;
      
      damage(ch, ch, dam, SPELL_ENERGY_DRAIN);  
  } else {
      send_to_char("Your energy drain backfires and almost hits you!\n\r",ch);
    }
    return;
  }
  ...
  if(damage(ch, victim, dam, SPELL_ENERGY_DRAIN))
    ...
    act("You tried to drain $N's energy, but you couldn't!",FALSE,ch,0,victim,TO_CHAR);
    act("$n tried to drain $N, but failed!",FALSE,ch,0,victim,TO_ROOM);
    act("$n tried to drain you.",FALSE,ch,0,victim,TO_VICT);
  }
 }
                      ...
                      struct char_data *victim, struct obj_data *obj)
{
  ...
  if (!wilderness(ch)) {
      send_to_char ("You must be in the wilderness to blend with the foliage.\n\r",ch);
      return;
  }
                       ...
                       struct char_data *victim, struct obj_data *obj)
{
  ...
  if (!UNDERWATER(ch))
    area_affect (level, dam, ch, SPELL_FIRE_BREATH, IMM_FIRE,
         "Searing flames scorch everyone and everything!",
	 "You feel a blast of hot air as the temperature rises dramatically!");
  else
    area_affect (level, dam, ch, SPELL_FIRE_BREATH, IMM_FIRE,
                        ...
                        struct char_data *victim, struct obj_data *obj)
{
  ...
  area_affect (level, dam, ch, SPELL_FROST_BREATH, IMM_COLD,
    "Numbing frost blasts everyone and everything!",
    "You feel a blast of cold air as the temperature drops dramatically!");
}
                       ...
                       struct char_data *victim, struct obj_data *obj)
{
  ...
  area_affect (level, dam, ch, SPELL_ACID_BREATH, IMM_ACID,
    "Scorching acid burns everyone and everything!",
    "You are nearly overwhelmed by the pungent stench of acid nearby!");
}
                            ...
                            struct char_data *victim, struct obj_data *obj)
{
  ...
  area_affect (level, dam, ch, SPELL_LIGHTNING_BREATH, IMM_ELEC,
         "Blinding bursts of electricity hit everyone and everything!",
         "You hear the loud crackle of intense electricity nearby!");
  else
    area_affect (level, dam, ch, SPELL_LIGHTNING_BREATH, IMM_ELEC,
	 "Electrical Energy surges through the water around you!",
