      ...
      if (ch->in_room==vict->in_room)
	{
	  if(ch!=vict && !in_group(ch,vict))
		...
		send_to_char( "Some puny mortal tries to hurt you.\n\r", vict);
	      else if(can_pkill(ch, vict) || !is_following(ch,vict))
		{
...
void do_hide(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (number(1,101) > ch->skills[SKILL_HIDE].learned) {
...
void do_sneak(struct char_data *ch, char *arg, int cmd)
{
  ...
  if (number(1,101) > ch->skills[SKILL_SNEAK].learned) {
...
void do_brew (struct char_data *ch, char *argument, int cmd)
{
  static struct brew_stuff
  {
    ...
    {
      { "light",	0,	8718,	1,	"cure light" },
      { "blind",	5,	8721,	4,	"cure blindness" },
      { "aura",         12,     8747,   7,      "sense aura" },
      { "life",         13,     8745,   8,      "sense life" },
      { "poison",	4,	8730,	9,	"remove poison" },
      { "true",		9,      8741,   10,     "true sight" },
      { "serious",	2,	8719,	10,	"cure serious" },
      { "infra",        19,     8750,   13,     "infravision" },
      { "bless",        14,     8742,   15,     "blessing" },
      { "shield",       16,     8732,   16,     "shield" },
      { "skin",         15,     8723,   17,     "stone skin" },
      { "strength",	6,	8722,	18,	"strength" },
      { "refresh",	7,	8724,	19,	"refresh" },
      { "critic",	1,	8727,	20,	"cure critic" },
      { "dinvis",       21,     8740,   20,     "detect invisible" },
      { "invis",        20,     8736,   20,     "invisibility" },
      { "hero",         8,      8735,   21,     "heros feast" },
      { "fly",          17,     8738,   23,     "fly" },
      { "water",        18,     8737,   24,     "waterbreath" },
      { "mana",         10,     8726,   25,     "mana gain" },
      { "heal",		3,	8729,	30,	"heal" },
      { "garmor",       26,     8734,   32,     "group armor" },
      { "gtrue",        23,     8748,   35,     "group true sight" },
      { "gdinvis",      25,     8743,   35,     "group detect invisable" },
      { "gfly",         27,     8746,   36,     "group fly" },
      { "ginvis",       24,     8744,   37,     "group invisibility" },
      { "fire",         22,     8731,   40,     "fireshield" },
      { "sanc",         11,     8739,   45,     "sanctuary" },
      { 0,		0,	0,	0,      0 }
    };
  int compnum;
  ...
  if(!ptr->name) {
    send_to_char("Have you been drinking your messed up brews?\n\r",ch);
  ...
  if (number(1,101)>ch->skills[SKILL_BREW].learned && !IS_IMMORTAL(ch)) {
    ...
    if (number(1,10)==1) {
      ...
      damage(ch, ch, dice(1, GetMaxLevel(ch)), SKILL_BREW);
  ...
  if(!(obj=make_object(ptr->index + BREW_INDEX, VIRTUAL|NORAND))) {
    send_to_char("This skill is experiencing technical difficulty.\n\r", ch);
    sprintf(buf, "screwup in brew, potion #%d", ptr->index);
    log_msg(buf);
    return;
  }
...
void do_hypnosis (struct char_data *ch, char *argument, int command)
{
  ...
  if (ch->skills[SKILL_HYPNOSIS].learned<number(1,101)) {
    ...
    if (IS_PC(ch)) {
      printf("[Debug] saving-throw modifier: %d\n", mod);
    }
...
void do_thrust (struct char_data *ch, char *argument, int command)
{
  ...
  if (ch->skills[SKILL_THRUST].learned < dice (1,101)) {
    ...
    send_to_char ("You've botched the thrust, watch out!\n\r",ch);
    act ("$n takes a wild lunge at $N but falls on $s face!",TRUE,ch,0,target,TO_NOTVICT);
    act ("$n lunges at you but falls on $s face!",TRUE,ch,0,target,TO_VICT);
  ...
  else
    {
      ...
      if (SkillSave (RANGER_LEVEL_IND,ch,target))
	dam /= 2;
...
void do_blessing(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (ch->specials.alignment<350) {
    send_to_char ("Your diety doen't like the way you've behaved.\n\r",ch);
    return;
  }
...
int pulse_effect(struct spevent_type *ps, long now) {
  ...
  EACH_CHARACTER(iter, vict) {
    if (ps->in_room == vict->in_room) {
      if((ps->caster!=vict) && !in_group(ps->caster,vict) && !IS_IMMORTAL(vict))
	if(!is_following(ps->caster,vict))
	  switch(psex->left) {
	  case 1:
	    /* first wave */
...
int general_aura_event(struct spevent_type *as, long now) {
  ...
  if(!check_peaceful(as->caster, "")) {
    for(vict=real_roomp(as->caster->in_room)->people; vict; vict=next) {
      ...
      if(vict && (as->caster != vict)) {
	if(!IS_IMMORTAL(vict) && (!is_following(as->caster, vict))) {
	  if(asex->dam_func)
