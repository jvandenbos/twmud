...
void boot_pose_messages(void)
{
  ...
  for (counter = 0;;counter++)
  {
    ...
    if (pose_messages[counter].level < 0)
      break;
		for (clss = 0;clss < 4;clss++)
		{
			pose_messages[counter].poser_msg[clss] = fread_action(fl);
			pose_messages[counter].room_msg[clss] = fread_action(fl);
		}
	}
...
void do_pose(struct char_data *ch, char *argument, int cmd)
{
    ...
    if ((level < pose_messages[0].level) || IS_NPC(ch))	{
        send_to_char("You can't do that.\n\r", ch);
        return;
    }

    for (counter = 0; (pose_messages[counter].level < level) &&
         (pose_messages[counter].level > 0); counter++);
    counter--;

    to_pose = number(0, counter);

    act(pose_messages[to_pose].poser_msg[0], FALSE, ch, 0, 0, TO_CHAR);
    act(pose_messages[to_pose].room_msg[0], FALSE, ch, 0, 0, TO_ROOM);
}

void do_restring(struct char_data *ch, char *arg, int cmd) {
   send_to_char("You may only restring in a clan shop\n\r", ch);
   if(TRUST(ch)>0)
     send_to_char("I believe you are looking for the 'string' command.\n\r", ch);
   return;
}

void do_forge(struct char_data *ch, char *arg, int cmd) {
   send_to_char("There is no clan forger here\n\r", ch);
   return;
}
